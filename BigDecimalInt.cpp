...
///in case of char* call the then convert to string and call our string constructor
BigDecimalInt::BigDecimalInt(char* str):BigDecimalInt(string(str)){}
...
///apply copy constructor to avoid common issues(shallow copy)
...
BigDecimalInt BigDecimalInt::operator-(BigDecimalInt anotherDec)
{
    ///we are here depend on that, number one left must be greater than right one
    ...
    ///if they are differ in the sign then it's a addition problem so call the addition method and let it do its work
